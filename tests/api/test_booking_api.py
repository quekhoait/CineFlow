...
@pytest.mark.parametrize("id_show, code_seats, errors, status_code", [
    (1, ["A1"], None, 201),
    (None, ["A1"], ValidationError, 400),
    (1, None, ValidationError, 400),
    (1, [], ValidationError, 400),
    (1, ["A1"], Exception, 500),
])
@patch('app.api.booking_api.booking_service')
@patch('flask_jwt_extended.view_decorators.verify_jwt_in_request')
def test_create_booking(mock_jwt, mock_service, client, id_show, code_seats, errors, status_code):
    mock_jwt.return_value = None
    payload = {
        'id_show': id_show,
